  ...
  run: async ({ m, sock }) => {
    if (!m.args[0])
      return sock.sendMessage(m.from, {
        text: `Contoh : ${m.prefix}igstalk _url_`,
      });
    const message = new Message({ m, sock });
    message.react(react.process);
    axios
      .get(
        apis.velixs.endpoint +
          `/instagram/download?apikey=${apis.velixs.apikey}&url=${m.args[0]}`,
      )
      .then(async (res) => {
        let result = res.data;
        result.data.url.forEach(async (url) => {
          ...
          if (buffer.headers["content-disposition"].includes("jpeg")) {
            ...
            await sock.sendMessage(
              m.from,
              { video: buffer.data, gifPlayback: true },
              { quoted: m },
            );
          }
      ...
      .catch(async (err) => {
        ...
        if (err.response?.data?.message == "Link not found")
